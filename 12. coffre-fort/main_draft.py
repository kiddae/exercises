...
def calculer_signaux(n, m, r, signaux, fils, questions):
    ...
    for q in questions:
        ...
        while i != q['puce b']:
            # trouve = False
            # print(i)
            # print([x for x in fils if x not in faux_chemins and x not in parcouru])
            # print(i)
            # fils_parcours = [
            #     x for x in fils if x not in faux_chemins and x not in parcouru and i in x.values()]
            fils_parcours = [
                x for x in liaisons[i] if x not in parcouru]
            for f in fils_parcours:
                if chemin+[f['puce2'] if i == f['puce1'] else f['puce1']] in faux_chemins:
                    continue
            ...
            if fils_parcours == []:
                # print(parcouru)
                # print(chemin)
                # fils_parcours.remove(
                #     {'puce1': chemin[-2], 'puce2': chemin[-1]})
                # faux_chemins += parcouru
                # faux_chemins.append(dernier_f)
                faux_chemins.append(chemin)
                # print('Erreur...')
                # print(dernier_f)
                # print(chemin)
                # print(faux_chemins)
        ...
        # print('erreurs:', faux_chemins)
        # print('chemin:', chemin)
        # print('\n')
    # Calcul du signal pour chaque
    # for i in chemins:
    #     p = 1
    #     for x in [signaux[n] for n in i]:
    #         p *= x
    #     print(p % 1671404011)
